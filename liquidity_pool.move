...
module lp_account::liquidity_pool {
    ...
    */  
    public entry fun create_liquidity_pool<CoinA, CoinB>() acquires State {
        assert_liquidity_pool_not_exist<CoinA, CoinB>(get_resource_account_address());
        assert_are_coin_types_existed<CoinA, CoinB>();
        assert_coins_are_sorted_not_equal<CoinA, CoinB>();

        let state_ref = borrow_global_mut<State>(get_resource_account_address());
        let account = account::create_signer_with_capability(&state_ref.signer_cap);
        let coin_a_symbol = coin::symbol<CoinA>();
        let coin_b_symbol = coin::symbol<CoinB>();
        let lpcoin_name = string_utils::format2(&b"{}-{} LP token", coin_a_symbol, coin_b_symbol);

        coin_a_symbol = get_truncated_symbol_name(coin_a_symbol);
        coin_b_symbol = get_truncated_symbol_name(coin_b_symbol);
        let lpcoin_symbol = coin_a_symbol;
        string::append(&mut lpcoin_symbol, string::utf8(b"-"));
        string::append(&mut lpcoin_symbol, coin_b_symbol);
        ...
        coin::destroy_freeze_cap<LPCoin<CoinA,CoinB>>(freeze_cap);
        coin::register<LPCoin<CoinA,CoinB>>(&account);

        let lp = LiquidityPool<CoinA, CoinB> {
        ...
        let name_a = coin::name<CoinA>();
        let name_b = coin::name<CoinB>();
        //emit create liquidity pool event
        let event = CreateLiquidityPoolEvent {
            coin_a: name_a, 
            coin_b: name_b,
            lp_coin: lpcoin_name,
            creation_timestamp_seconds: timestamp::now_seconds()    
        };
        event::emit_event(&mut state_ref.create_liquidity_pool_events, event);
    }
    ...
    */
    public fun supply_liquidity<CoinA, CoinB>(
        ...
        assert_liquidity_pool_exists<CoinA, CoinB>(resource_address);
        assert_coins_are_sorted_not_equal<CoinA, CoinB>();

        let lp_ref = borrow_global_mut<LiquidityPool<CoinA,CoinB>>(resource_address);
        let state_ref = borrow_global_mut<State>(resource_address);
        let is_initial_supply = is_initial_liquidity_supply<CoinA,CoinB>();
        let coin_a_supply = coin::value<CoinA>(&coin_a);
        let coin_b_supply = coin::value<CoinB>(&coin_b);
        let _mint_amount = 0;

        if (is_initial_supply) {     
           _mint_amount = (math128::sqrt(math128::mul_div((coin_a_supply as u128),(coin_b_supply as u128),1)) as u64);  
           assert_liquidity_above_minimum_liquidity(_mint_amount, true);
           let minted_for_resource = coin::mint<LPCoin<CoinA,CoinB>>(MINIMUM_LIQUIDITY, &lp_ref.lp_coin_mint_cap);
            //transfer minimum lp coin to resource address 
           coin::deposit<LPCoin<CoinA,CoinB>>(resource_address, minted_for_resource);    
           _mint_amount = _mint_amount - MINIMUM_LIQUIDITY;    
        } else {
            let lp_total_supply = (*option::borrow(&coin::supply<LPCoin<CoinA,CoinB>>()) as u64);
            _mint_amount = math64::min(
                math64::mul_div(coin_a_supply, lp_total_supply, coin::value<CoinA>(&lp_ref.coin_a_reserve)),
                math64::mul_div(coin_b_supply, lp_total_supply, coin::value<CoinB>(&lp_ref.coin_b_reserve)));
        };

        let return_minted = coin::mint<LPCoin<CoinA,CoinB>>(_mint_amount, &lp_ref.lp_coin_mint_cap); 

        //add both supplied coins to reserve
        coin::merge<CoinA>(&mut lp_ref.coin_a_reserve, coin_a);
        coin::merge<CoinB>(&mut lp_ref.coin_b_reserve, coin_b);   

        //emit supply liquidity event
        let event = SupplyLiquidityEvent {
            coin_a: coin::name<CoinA>(), 
            coin_b: coin::name<CoinB>(),
            amount_a: coin_a_supply,
            amount_b: coin_b_supply,
            lp_amount: _mint_amount,
            creation_timestamp_seconds: timestamp::now_seconds()
        };
        event::emit_event(&mut state_ref.supply_liquidity_events, event);
    ...
    */
    public fun remove_liquidity<CoinA, CoinB>(
        ...
        let lp_total_supply = (*option::borrow(&coin::supply<LPCoin<CoinA,CoinB>>()) as u64);
        let lp_redeem_amount = coin::value<LPCoin<CoinA,CoinB>>(&lp_coins_to_redeem);
        let coin_a_amount = math64::mul_div(
            lp_redeem_amount,
            coin::value<CoinA>(&lp_ref.coin_a_reserve),
            lp_total_supply);

        let coin_b_amount = math64::mul_div(
            lp_redeem_amount,
            coin::value<CoinB>(&lp_ref.coin_b_reserve),
            lp_total_supply);

        assert_coins_to_return_above_zero(coin_a_amount, coin_b_amount);

        //burn the redeemed lp coins
        coin::burn<LPCoin<CoinA,CoinB>>(lp_coins_to_redeem, &lp_ref.lp_coin_burn_cap);

        let return_coin_a = coin::extract<CoinA>(&mut lp_ref.coin_a_reserve, coin_a_amount);
        let return_coin_b = coin::extract<CoinB>(&mut lp_ref.coin_b_reserve, coin_b_amount);

        //emit remove liquidity event
        let event = RemoveLiquidityEvent {
            coin_a: coin::name<CoinA>(), 
            coin_b: coin::name<CoinB>(),
            lp_amount: lp_redeem_amount,
            amount_a: coin_a_amount,
            amount_b: coin_b_amount,
            creation_timestamp_seconds: timestamp::now_seconds()
        };
        event::emit_event(&mut state_ref.remove_liquidity_events, event);

        return (return_coin_a, return_coin_b)
    }
    ...
    */
    public fun swap<CoinA, CoinB>(
        ...
        assert_coins_to_swap_above_zero(amount_coin_a_in, amount_coin_b_in);
        

        let lp_ref = borrow_global_mut<LiquidityPool<CoinA,CoinB>>(resource_address);
        let state_ref = borrow_global_mut<State>(resource_address);     

        let amount_a_reserve = coin::value<CoinA>(&lp_ref.coin_a_reserve);
        let amount_b_reserve = coin::value<CoinB>(&lp_ref.coin_b_reserve);
        let current_k = math64::mul_div(amount_a_reserve,amount_b_reserve,1);
        let new_k = math64::mul_div(
            amount_a_reserve + amount_coin_a_in - amount_coin_a_out,
            amount_b_reserve + amount_coin_b_in - amount_coin_b_out,
            1
        );

        assert!(new_k >= current_k, ECodeForAllErrors);

        coin::merge<CoinA>(&mut lp_ref.coin_a_reserve, coin_a_in);
        coin::merge<CoinB>(&mut lp_ref.coin_b_reserve, coin_b_in);
        let return_coin_a = coin::extract<CoinA>(&mut lp_ref.coin_a_reserve, amount_coin_a_out);
        let return_coin_b = coin::extract<CoinB>(&mut lp_ref.coin_b_reserve, amount_coin_b_out);

        //emit swap liquidity event
        let event = SwapEvent {
            coin_a: coin::name<CoinA>(), 
            coin_b: coin::name<CoinB>(),
        ...
        event::emit_event(&mut state_ref.swap_events, event);
    ...
    */
    ...
    /*
        Checks whether it's the initial liquidity supply
    */
    inline fun is_initial_liquidity_supply<CoinA,CoinB>(): bool {
        coin::balance<LPCoin<CoinA,CoinB>>(get_resource_account_address()) < MINIMUM_LIQUIDITY
    }

    /*
        Gets the truncated coin symbol name (up to 4 chars)
    */
    inline fun get_truncated_symbol_name(name: String): String {
        let s = name;
        if (string::length(&s) > 4) {
            s = string::sub_string(&s, 0, 4)
        };
        s
    }
    ...
    //==============================================================================================
    inline fun assert_liquidity_pool_not_exist<CoinA, CoinB>(
        resource_addr: address) 
        acquires LiquidityPool {
        assert!(!exists<LiquidityPool<CoinA, CoinB>>(resource_addr), ECodeForAllErrors);
    }

    inline fun assert_liquidity_pool_exists<CoinA, CoinB>(
        resource_addr: address) 
        acquires LiquidityPool {
        assert!(exists<LiquidityPool<CoinA, CoinB>>(resource_addr), ECodeForAllErrors);
    }

    inline fun assert_are_coin_types_existed<CoinA, CoinB>() {
        assert!(coin::is_coin_initialized<CoinA>(), ECodeForAllErrors);
        assert!(coin::is_coin_initialized<CoinB>(), ECodeForAllErrors);
    }


    inline fun assert_coins_are_sorted_not_equal<CoinA, CoinB>() {
        let name_coin_A = type_info::type_name<CoinA>();
        let name_coin_B = type_info::type_name<CoinB>();

        let compare_result = comparator::compare(&name_coin_A, &name_coin_B);

        assert!(!comparator::is_equal(&compare_result), ECodeForAllErrors);
        assert!(comparator::is_smaller_than(&compare_result), ECodeForAllErrors);
    }

    inline fun assert_liquidity_above_minimum_liquidity(lp_amount: u64, is_initial: bool) {
        assert!(lp_amount > 0, ECodeForAllErrors);
        if (is_initial) {
            assert!(lp_amount > MINIMUM_LIQUIDITY, ECodeForAllErrors);
        }
    }

    inline fun assert_coins_to_return_above_zero(
        coin_a: u64,
        coin_b: u64) {
        assert!(coin_a > 0 && coin_b > 0, ECodeForAllErrors);    
    }

    inline fun assert_coins_to_swap_above_zero(
        coin_a: u64,
        coin_b: u64) {
        assert!(coin_a > 0 || coin_b > 0, ECodeForAllErrors);    
    }
